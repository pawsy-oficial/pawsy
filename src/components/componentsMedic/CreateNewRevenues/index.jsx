...
export function CreateNewRevenues({ idPet, idClinic, idMedic }) {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_URL}/get-type-revenue`)
      .then((result) => {
        console.log(result);
        setTypeRevenue(result.data.results);
        axios
          .get(`${import.meta.env.VITE_URL}/get-revenues/1`)
          .then((e) => {
            setDataHeader({
    ...
    drug: Yup.array().of(
      Yup.object().shape({
        ...
        concentration: Yup.number()
          .required("Campo obrigatório")
          .typeError("Deve ser um numero")
          .positive("Deve ser maior que zero (0)"),
        dosage: Yup.string()
          .min(5, "Deve ter no minimo 5 caracteres")
          .max(260, "Limite atingido (260 caracteres)")
          .required("Campo obrigatório"),
        drugName: Yup.string()
  ...
  const onSubmit = (data) => {
    ...
    data[
      "description"
    ] = `Uma nova receita de medicamento foi adicionada ao plano de tratamento de seu pet, visando sua saúde e bem-estar.`;
    console.log(data);

    axios
      .post(`${import.meta.env.VITE_URL}/revenues`, data)
      .then((e) => {
        axios
  ...
  return (
    ...
    >
      <main className="min-h-[892px] flex flex-col max-h-fit bg-white border border-primary relative">
        <img
          src={LogoWaterMark}
        ...
        />
        <header className="flex py-6 px-8 w-full bg-[#F5FFFE] text-xs font-bold flex-col gap-2">
          <label className="flex flex-row items-center gap-4">
            <span>Tipo da receita:</span>
            <select
            ...
            >
              {typeRevenue.map((tp) => {
                return (
                  <option value={tp.id_TipoReceita}>{tp.nm_TipoReceita}</option>
                );
              })}
            </select>
          </label>

          <label className="flex flex-row items-center gap-4 text-zinc-900">
        ...
        <div className="w-44 h-[1px] my-20 mx-auto bg-black">
          <div className="flex flex-col items-center pt-3">
            <span className="text-xs">Dr(a) {dataHeader.nm_medico}</span>
            <span className="text-[9px]">CRMV: {dataHeader.cd_crmv}</span>
          </div>
        </div>

        <footer className="w-[595px] h-8 bg-primary flex justify-center items-center absolute bottom-0">
          <img src="src/img/logo.png" alt="Logo da Pawsy" className="w-16" />
        </footer>
      </main>
      ...
      <button
        ...
        className={`bg-primary py-1 px-6 rounded-lg text-white font-bold font-lato ${
          fields.length == 0 && "disabled:opacity-25"
        }`}
      >
        Salvar
      </button>
