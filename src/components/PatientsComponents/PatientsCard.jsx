  ...
  return (
    <>
      ...
      <button
        onClick={() => {
          setSee(!see);
          console.log(id, pet, image, donosP);
        }}
      >
...
function ModalSeePatient({ See, setSee, donos, id, pet, image }) {
  function handleRemovePatient() {
    axios
      .get(`${import.meta.env.VITE_URL}/profileClinic`, {
        headers: {
      ...
      .then((e) => {
        axios
          .delete(
            `${import.meta.env.VITE_URL}/patient/${
              e.data.storedIdClinica
            }/${id}`,
            {
              headers: {
                Authorization: `Bearer ${Cookies.get("jwtTokenClinic")}`,
  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg p-4 min-w-fit w-96 relative flex flex-col gap-6">
        <button onClick={() => setSee(!See)} className="absolute right-4">
          <X size={28} />
        </button>
        <div className="flex gap-6">
          ...
          <div className="flex flex-col gap-8 justify-center">
            <div className="flex flex-col items-start gap-4">
              <h1 className="text-3xl font-bold capitalize">{pet}</h1>
              <p className="text-normal flex gap-2">
                Tutor:
                <span className="font-semibold">{donos}</span>
              </p>
            </div>
          </div>
        </div>
...
const memoPatienrsCard = memo(PatientsCard);
export { memoPatienrsCard as PatientsCard };
